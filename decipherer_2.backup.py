"""decipherer_2.backup.py - After taking logarithm

This code is a little bit more sophisticated than the previous decipherer.py
"""
...
class LanguageTrait:
    def __init__(self, corpus):
        ...
        Example:
            ...
            english_trait = LanguageTrait(corpus=war_and_peace_text)
        """
        self.alphabets = []
        self.frequencies = {}
    ...
    def compute_log_score_of(self, text):
        """
        Example:
            english_trait.compute_log_score_of("This is a sunny day!"))  # larger
            english_trait.compute_log_score_of("this si a bunny ady!"))  # smaller
        """
        score = 0
...
def decipher(language_trait, scrambled_text, iterations):
    # Run Metropolis-Hasting algorithm:
    current_map = {alphabet: alphabet for alphabet in language_trait.alphabets}
    current_log_score = language_trait.compute_log_score_of(scrambled_text)
    ...
    for _ in range(iterations):
        ...
        next_log_score = language_trait.compute_log_score_of(maybe_next_text)
        should_accept = (
            True
...
if __name__ == "__main__":
    ...
    with open(CORPUS_FILE_PATH) as corpus_file:
        ...
        english_trait = LanguageTrait(corpus=english_corpus)
        debug("main(): reading corpus... Done.")
        debug(
            "main(): this is a sunny day!",
            english_trait.compute_log_score_of("this is a sunny day!"),
        )  # larger
        debug(
            "main(): htis si a bunny ady!",
            english_trait.compute_log_score_of("htis si a bunny ady!"),
        )  # smaller
    ...
    initial_log_score = english_trait.compute_log_score_of(scrambled_text)
    debug("main(): scrambled_text (first 128 chars): ", scrambled_text[:128])
    print("Start?", end="")
    input()
    # Run Metropolis-Hasting algorithm:
    decipher_started_at = time.time()
    decipher(english_trait, scrambled_text, iterations=30000)
