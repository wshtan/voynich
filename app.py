...
def move_one_step_with_two_transpositions(p_map):
    keys = az_list()
    ...
    p_map_2[sample[2]] = p_map[sample[3]]
    return p_map_2
...
def compute_statistics(filename):
    ...
    """
    debug("compute_statistics(): hello:", time.time())
    ...
    debug("compute_statistics(): hello - 2:", time.time())
    i = 0
    while i < len(data) - 1:
    ...
    debug("compute_statistics(): hello - 3:", time.time())
    transition_matrix /= np.sum(transition_matrix, axis=1, keepdims=True)
    debug("compute_statistics(): bye: ", time.time())
...
def get_initial_state_from_frequencies(text, transition_maitrx, frequency_statistics, char_to_ix):
    pass
...
def propose_a_move_with_two_transpositions(state):
    """
    Proposes a new move for the given state,
    by moving one step (randomly swapping two characters)
    """
    ...
    for key, value in iter(state.items()):
        new_state[key] = value
    new_state["permutation_map"] = move_one_step_with_two_transpositions(state["permutation_map"])
    return new_state
...
def main(argv):
    ...
    fig.suptitle("negative log likelihood vs iterations: Comparing 1 transposition vs 2 transpositions at once")
    # One transposition:
    lpss_1tp = []
    for i in range(NN):
        ...
        debug(f"main(): one transposition: [{i+1} / 10]: press enter to start...")
        #input()
        debug("main(): start decipher. time: {decipher_started_at}")
    ...
    for i in range(NN):
        ax1.plot(
            ...
            "-",
            color="#00f",
            label=("lps vs iters (one transposition)" if i == 0 else None)
        )
    # Two transpositions:
    lpss_2tp = []
    for i in range(NN):
        ...
        tolerance = float(options.tolerance)
        decipher_started_at = time.time()
        debug(f"main(): two transpositions: [{i+1} / 10]: press enter to start...")
        #input()
        debug("main(): start decipher. time: {decipher_started_at}")
        state, lps, _ = metropolis_hastings(
            initial_state,
            propose_a_move_with_two_transpositions,
            compute_probability_of_state,
            iters=iters,
            print_every=print_every,
            tolerance=tolerance,
    ...
    for i in range(NN):
        ax2.plot(
            ...
            "-",
            color="#f00",
            label=("lps vs iters (two transpositions)" if i == 0 else None)
        )
